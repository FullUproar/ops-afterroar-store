...
| `seidr` | Phase 0 (full pipeline + matcher + explainer + CLI) | Sprints 1.0.18→1.0.20: schema, LLM pipeline, validator, 7 hand-authored reference profiles, cosine matcher with subtle-wrongness suite, explanation generator, and offline CLI runner. **End-to-end recommendation flow works**, runnable from a shell. 154/154 tests pass. Top-500 LLM run (laptop, requires API key) is the only remaining gap. | Profile-driven; quiz → 24-dim vector → cosine match → ranked recommendations with natural-language explanations |
