...
| `seidr` | Phase 0 (pipeline + matcher + 7 reference profiles) | Sprint 1.0.18 + 1.0.19 land schema, LLM pipeline, validator, 7 hand-authored reference profiles, and the cosine matcher with full subtle-wrongness suite. 131/131 tests pass. Top-500 LLM run (1.0.20) and recommendations module (1.0.21) come next. | Profile-driven; quiz → 24-dim vector → cosine match to game profiles. Complementary to mimir, not a replacement |
