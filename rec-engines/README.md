...
- [`seidr/`](./seidr/) — Old Norse magic and divination. **Phase 0 research + deployable static quiz UI + game-profile pipeline.** Profile-driven engine: 18-question quiz emits a 24-dim player vector, cosine-matched against game profiles. 50 curated questions, 24 dimensions, deployable HTML quiz at `seidr/quiz-ui/`. Sprint 1.0.18 added the schema (rec_seidr_*), LLM pipeline (`scripts/profile-game.mjs`), validator, and 7 hand-authored reference profiles for fixture games. 96/96 seidr tests pass. Cosine matcher comes next.
